...
int main(int argc,char* argv[]){
    ...
    int t = (36%num==0) ? 36/num : 36/num+1; 
    // target[33]=0;
    if( rank == 0 ) {
      fflush(stdout);
      scanf("%s",target);
    ...
    MPI_Bcast(&target, 33, MPI_CHAR, 0, MPI_COMM_WORLD);

    char s[10];
    char output[40];

    for( int i=rank*t ; i<36&&i<(rank+1)*t ; i++ ) {
      for(int j=0;j<36;j++) {
        for(int k=0;k<36;k++) {
          for(int w=0;w<36;w++) {
            s[0] = list[i]; s[1] = list[j]; s[2] = list[k]; s[3]=list[w]; s[4]='\0';
            MDString(s,output);
